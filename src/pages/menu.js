...
import menuBg from "../images/fettucine.jpg";
import {
  MenuStarter,
  MenuMain,
  MenuDessert,
...
const Menu = (props) => {
  ...
  if (starterActive) {
    BgColorStarter = { backgroundColor: "rgb(1, 191, 113)", color: "white" };
    BgColorMain = { backgroundColor: "" };
    BgColorDessert = { backgroundColor: "" };
  } else if (mainActive) {
    BgColorMain = { backgroundColor: "rgb(1, 191, 113)", color: "white" };
    BgColorStarter = { backgroundColor: "" };
    BgColorDessert = { backgroundColor: "" };
  } else if (dessertActive) {
    BgColorDessert = { backgroundColor: "rgb(1, 191, 113)", color: "white" };
    BgColorStarter = { backgroundColor: "" };
    BgColorMain = { backgroundColor: "" };
  }
    ...
    <>
      <MenuDiv id="menu">
        ...
        <MenuWrapper>
          <ButtonsWrapper>
            <MenuButton
              active={BgColorStarter}
              func={activateStarter}
              name={"starter"}
            ></MenuButton>
            <MenuButton
              active={BgColorMain}
            ...
            ></MenuButton>
            <MenuButton
              active={BgColorDessert}
              func={activateDessert}
              name={"dessert"}
            ></MenuButton>
          </ButtonsWrapper>
          <MenuItemWrapper>
            {activeValue.map((data) => (
              <MenuItemDiv key={data.id}>
                ...
                <ItemDescription>{data.itemDescription}</ItemDescription>
              </MenuItemDiv>
            ))}
          </MenuItemWrapper>
        </MenuWrapper>
